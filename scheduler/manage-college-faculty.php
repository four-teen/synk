                ...
                <tbody>
                  <tr>
                    <td colspan="5" class="text-center text-muted">
                      Please select a college to view assigned faculty.
                    </td>
                  </tr>
                </tbody>
              </table>
...
<!--           <div class="mb-3">
            ...
            <input type="text" id="selectedCollegeName" class="form-control" readonly>
          </div> -->
      ...
      <div class="modal-footer">
        ...
        <button class="btn btn-primary" id="btnSaveAssignment">Save</button>
      </div>
...
$('#faculty_id').select2({
  ...
  // ------------------------------------
  // Load faculty dropdown on page load
  // ------------------------------------
  loadFacultyDropdown();

  // Auto-select first college
  // let firstCollege = $('#college_id option:eq(1)').val();
  // if (firstCollege) {
  //   $('#college_id').val(firstCollege).trigger('change');
  // }
  ...
  $('#btnAddFaculty').on('click', function() {
    ...
    const college_text = $('#college_id option:selected').text();
    ...
    $('#selectedCollegeName').val(college_text);
    $('#faculty_id').val(null).trigger('change');

    var modal = new bootstrap.Modal(document.getElementById('addFacultyModal'), {
  ...
  $('#btnSaveAssignment').on('click', function() {
    ...
    if (!college_id || !faculty_id) {
      Swal.fire("Missing Data", "Please select faculty to assign.", "warning");
      return;
    }
    ...
    $.ajax({
      ...
      success: function(res) {
        if (res.status === "inserted") {
          ...
          Swal.fire("Already Assigned", "This faculty is already assigned to the selected college.", "info");
        }
        else {
...
// ======================================================================
// LOAD ASSIGNED FACULTY BY COLLEGE
// ======================================================================
function loadCollegeFaculty(collegeID = null) {
  const college_id = collegeID || $('#college_id').val();

  if (!college_id) {
    $('#collegeFacultyTable tbody').html(
      '<tr><td colspan="5" class="text-center text-muted">Please select a college to view assigned faculty.</td></tr>'
    );
    return;
  }
    ...
    success: function(data) {
      ...
      if (data.length === 0) {
        rows = `
          <tr>
            <td colspan="5" class="text-center text-muted">
              No faculty assigned to this college yet.
            </td>
          </tr>`;
      } else {
        let n = 1;
        data.forEach(row => {
          rows += `
            <tr>
              ...
              <td class="text-end">
                <button class="btn btn-sm btn-danger btn-remove-assignment" data-id="${row.college_faculty_id}">
      ...
      $('#collegeFacultyTable tbody').html(rows);
    }
  });
}
